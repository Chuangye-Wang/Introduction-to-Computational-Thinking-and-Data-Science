...
class RectangularRoom(object):
    ...
    def set_tile_dirt_amount(self):
        """
        Assign dirt_amount for every tile in RectangularRoom at beginning.
        
        Returns: a dictionary with dirtamount value for every tile position.
        """
        for i in range(0, self.width):
            for j in range(0, self.height):
                self.tiles[(i, j)] = self.dirt_amount
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        x1, y1 = math.floor(pos.get_x()), math.floor(pos.get_y())   #using math.floor() rather than np.floor() to get an int number
        new_dirt_amt = self.tiles[(x1, y1)] - capacity
        if capacity < 0:
            if new_dirt_amt > 1:
                new_dirt_amt = 1
        else:
    ...
    def get_dirt_amount(self, m, n):
        ...
        """
        #raise NotImplementedError
        if not self.is_position_in_room(Position(m, n)):
            raise ValueError('pos({},{}) are out of room space'.format(m,n))
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        x0 = self.room.width*random.random()
        y0 = self.room.height*random.random()
...
#            new_direct = self.get_robot_direction() + delta_angle
#            newpos = self.position.get_new_position(new_direct, self.speed)
#            new_x, new_y = newpos.get_x(), newpos.get_y()
#            if (new_x < self.room.width and new_x >= 0) and (new_y < self.room.height and new_y >= 0):
#                return newpos
        raise NotImplementedError
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_furnished(self, pos):
        ...
        if x1 == self.width or y1 == self.height:
            raise ValueError('x and y cannot be equal to width or height')
        x1 = math.floor(x1)
        y1 = math.floor(y1)
        if (x1, y1) in self.furniture_tiles:
            return True
        else:
            return False
    ...
    def is_position_valid(self, pos):
        ...
        if (x1 >= 0 and x1 < self.width) and (y1 >= 0 and y1 < self.height):
            if (math.floor(x1), math.floor(y1)) not in self.furniture_tiles:
                return True
            else:
                return False
        else: 
    ...
    def get_random_position(self):
        ...
        for key in self.tiles:
            if key not in self.furniture_tiles:
                a.append(key)
        x0, y0 = random.choice(a)
        x0 += random.random()
        y0 += random.random()

        return Position(x0, y0)
        #raise NotImplementedError
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        if self.room.is_position_valid(newpos):
            ...
            new_direct = 360*random.random()
            #a quicker way: new_direct = self.get_robot_direction() + 90 + 180*random.random()
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    if num_robots == 1:
        for trial in range(num_trials):
            ...
            timesteps = 0
            f = robots.room.get_num_cleaned_tiles() / robots.room.get_num_tiles()
            while(f < min_coverage):
                robots.update_position_and_clean()
                f = robots.room.get_num_cleaned_tiles() / robots.room.get_num_tiles()
                timesteps += 1
            totalnum.append(timesteps)
    else:
        for trial in range(num_trials):
            room = EmptyRoom(width, height, dirt_amount)
            robots = []
            ...
            timesteps = 0
            f = room.get_num_cleaned_tiles() / width / height
            while(f < min_coverage):
                for i in range(num_robots):
                    robots[i].update_position_and_clean()
                f = robots[i].room.get_num_cleaned_tiles() / robots[i].room.get_num_tiles()
                timesteps += 1
            totalnum.append(timesteps)
    return np.average(totalnum)
