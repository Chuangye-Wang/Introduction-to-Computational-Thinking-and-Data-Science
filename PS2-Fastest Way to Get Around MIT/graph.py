...
class Digraph(object):
    ...
    def __init__(self):
        self.nodes = set([])
        self.pureedges = {}  # must be a dict of Node -> list of edges
        self.edges ={}
    def __str__(self):
        edge_strs = []
    ...
    def get_pureedges_for_node(self, node):
        return self.pureedges[node]
    ...
    def add_edge(self, weightededge):
        """Adds an Edge or WeightedEdge instance to the Digraph. Raises a
        ValueError if either of the nodes associated with the edge is not
        in the  graph."""
        src = weightededge.get_source()
        dest = weightededge.get_destination()
        #totaldis = weightededge.get_total_distance()
        #outdoordis = weightededge.get_outdoor_distance()
        if not (src in self.edges and dest in self.edges):
            raise ValueError('Node not in graph')
        self.edges[src].append(str(weightededge))

    def add_pureedge(self, edge):
        """Adds an Edge or WeightedEdge instance to the Digraph. Raises a
        ValueError if either of the nodes associated with the edge is not
        in the  graph."""
        src = edge.get_source()
        ...
        #totaldis = weightededge.get_total_distance()
        #outdoordis = weightededge.get_outdoor_distance()
        if not (src in self.pureedges and dest in self.pureedges):
            raise ValueError('Node not in graph')
        self.pureedges[src].append(dest)
